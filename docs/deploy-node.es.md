# Despliegue en Linux con Node y SQLite

Aplicación familiar para convertir misiones y responsabilidades en progreso visible, sin rankings ni penalizaciones. En modo demo, Aina e Iara completan misiones y Xavi y Mireia las gestionan; el modo personalizado permite definir los adultos e hijos. La interfaz es responsive, usa tema oscuro por defecto, incluye castellano, catalán e inglés, historial, rachas, logros, una casa animada y entrada por voz del navegador. La voz pide confirmación antes de registrar una misión y no envía audio a un LLM.

Este documento detalla solo el destino Linux. El mismo código también se compila para Sites con D1; véase [la guía Sites](deploy-sites.md). El primer acceso ofrece modo personalizado y modo demo. La base local arranca sin actividad anterior.
...
- El destino Linux no necesita Docker, Workers ni D1.
...
- La API usa un contrato pequeño (`prepare/bind/first/run/batch`): `better-sqlite3` en Node, D1 en el build de Sites. Drizzle conserva el esquema y las migraciones.
...
El catálogo inicial incluye las misiones de ambas niñas. Hacer la cama y prepararse a tiempo dan 1 Xp al día cada una; mantener la habitación ordenada por la noche da 5 Xp; lavarse los dientes da 2 Xp por registro, hasta dos veces al día. Kids&Us Homework da 5 Xp de lunes a viernes. Siguen activas organizar el día (5 Xp diarios), la rutina de ducha (10 Xp diarios) y elegir la ropa de mañana (5 Xp diarios). La mochila del cole da 5 Xp de domingo a jueves. «Recoger mi ropa» se ha retirado.

Iara prepara la mesa para cenar de lunes a viernes (2 Xp), prepara la mochila extraescolar domingo, lunes y martes (5 Xp) y tiene la misión semanal de responsabilizarse de los deberes del cole (10 Xp). Aina recoge la mesa de la cena de lunes a viernes (2 Xp), prepara la mochila extraescolar lunes, martes y miércoles (5 Xp) y tiene la misión semanal de probar un alimento nuevo (10 Xp). Las misiones semanales se registran una vez por semana; las misiones se pueden editar desde la gestión parental.

Los bonus cuentan semanas de **lunes a domingo**, con fechas de `Europe/Madrid`; el cierre del ciclo de progreso se configura aparte. El servidor conserva una copia de las misiones programadas para cada semana. Los cambios de una misión existente empiezan a aplicarse al terminar la semana en curso: hasta entonces sus días, repeticiones y Xp siguen los del plan congelado. Una misión **nueva o recién aprobada** durante la semana está disponible desde ese día y cuenta para completar el **superbonus ×5**, sin exigir sus días anteriores; si se añade de martes a domingo, no puede obtener su propio bonus ×2 esa semana. Desde el lunes siguiente participa en ambos bonus normalmente. Solo la primera semana empieza a contar desde el primer acceso autenticado si la instalación se hizo a mitad de semana. En semanas posteriores se exigen todos los días programados desde el lunes, incluso si la app no se abrió durante una semana completa. Al completar una misión todos sus días asignados y repeticiones se concede en ese momento un bonus igual a sus Xp de esa semana (**×2 en total**). Al completar **todas** las misiones programadas se concede el superbonus de cuatro veces los Xp base (**×5 en total**), descontando los bonus ×2 ya concedidos: nunca se suman ambos multiplicadores. Las pausas programadas quitan los días afectados de los requisitos; una semana sin ninguna misión exigible no concede bonus. Las misiones de una sola vez y los descubrimientos no entran en el multiplicador. Las correcciones posteriores recalculan el bonus y el saldo.

El calendario marca en **plateado** los días de misiones que obtuvieron x2 y en **dorado** la semana con x5. El historial muestra el extra de Xp por separado y permite repetir la animación. La copa del logo celebra el bonus con confeti cuando se concede; cada perfil lo ve una vez, con alternativa estática si el navegador solicita menos movimiento. Los bonus son parte del saldo y de los niveles.

Con todas las misiones iniciales hechas, cada niña puede obtener **302 Xp base** por semana y **1510 Xp con el superbonus ×5**. El cálculo es: 267 Xp de misiones comunes, 10 Xp de su misión de la cena, 15 Xp de mochila extraescolar y 10 Xp de su misión semanal. Los descubrimientos quedan fuera de este máximo. Cada descubrimiento se concede una sola vez por niña y da 1 Xp; el servidor comprueba que exista en el catálogo y, para los vinculados al progreso, que se cumpla su condición. Los seis descubrimientos de interacción visual tienen un máximo cerrado por niña; el servidor no puede demostrar que se hizo un clic real en un navegador, así que esos seis siguen siendo recompensas lúdicas de confianza. El primero llega al completar una misión; los relacionados con una misión concreta o la hora requieren varias acciones para evitar una avalancha inicial. Un diálogo indica el nombre de cada descubrimiento nuevo y **Progreso** permite consultar los ya encontrados.

El **Superpremio** inicial, «Traer una amiga a dormir el fin de semana», cuesta **1500 Xp**. Puede solicitarse al acumular ese saldo por cualquier vía; una semana perfecta inicial da 1510 Xp, suficientes para comprarlo directamente si no se gastaron antes. Al solicitarlo se reservan los 1500 Xp, un adulto confirma la fecha y una cancelación devuelve el saldo. La familia puede editar o archivar el premio desde la gestión parental. El cambio del premio gratuito anterior se aplica una vez al estado JSON, sin cobrar canjes históricos ni cambiar el esquema SQLite.

La sección **Reglas** de la app explica con un esquema y el catálogo vivo de misiones cómo se consiguen los bonus y cómo funcionan el saldo y los premios. Las propuestas visuales para elegir la futura interfaz están en [UI proposals](ui-proposals/index.html). Son HTML de demostración y no modifican los datos familiares.
...
Abre `https://taskxp.example.net`. Elige modo personalizado (uno o más adultos y uno o más hijos) o modo demo (Xavi, Mireia, Aina e Iara), introduce la contraseña familiar (10–128 caracteres) y el token `TASK_XP_SETUP_TOKEN`. La app genera un código distinto por persona, cada uno con cuatro animales en orden, usando el catálogo que incluye el gato bengalí. Guárdalos en el gestor familiar de contraseñas. SQLite guarda los hashes Argon2id y una copia **cifrada** de los códigos para recuperarlos si se interrumpe esta pantalla; la copia se borra al primer login correcto con la contraseña familiar. Los códigos nunca se guardan en claro. Una instalación creada con una versión anterior cifra automáticamente los códigos pendientes al recibir la primera petición.
...
El script crea `/var/lib/taskxp/backups` con permisos `0700`, guarda una copia íntegra con permisos `0600`, comprueba su integridad y muestra la ruta creada. Incluye las transacciones aún pendientes en el WAL. No copies solo `taskxp.db` mientras la app está activa ni confíes en que una parada siempre vacíe el WAL. Conserva copias fuera de `erik` y ensaya restauraciones.

En **Configuración → Vuestros datos**, un padre puede exportar el progreso a JSON e importar ese mismo formato. La importación valida la estructura, limita los Xp de cada registro, comprueba las referencias a misiones, premios y descubrimientos, y reemplaza de forma atómica misiones, premios, Xp e historial; pide confirmar que se perderá el progreso actual. Los bonus semanales se recalculan a partir de los registros. El JSON no incluye contraseñas, códigos animales, sesiones ni dispositivos recordados, que permanecen configurados en la instalación de destino. Conserva también copias SQLite: el JSON no sustituye una copia completa de la base de datos.

**Configuración** reúne descansos, acceso familiar y el cierre del ciclo de progreso. Se puede elegir cualquier último día de ciclo; el cierre ocurre a medianoche en `Europe/Madrid`. Los ciclos ya cerrados conservan sus fechas. El calendario y los bonus semanales de misiones siguen de lunes a domingo: cambiar el cierre del progreso no altera sus requisitos. La corrección por día, semana o mes se aplica a la niña seleccionada. «Restablecer Xp» deja los registros a cero y visibles en el historial; «Deshacer actividad» revierte sus registros y los cambios de propuestas o canjes aún independientes, respetando ajustes y decisiones posteriores de los padres. Los bonus se recalculan. «Empezar de nuevo» exige dos confirmaciones y reinicia el progreso de ambas niñas y el catálogo inicial, pero conserva el acceso familiar. Los periodos se interpretan por fecha de Madrid; las semanas de estas acciones van de lunes a domingo.

## Desarrollo y legado de Cloudflare

La base D1 del Site publicado, la D1 local de Wrangler y la SQLite de Linux son independientes. La nueva SQLite arranca vacía. Tras crear el acceso familiar en Linux se puede importar manualmente un JSON exportado por la propia app si los miembros coinciden; las credenciales del Site no viajan en ese archivo.
