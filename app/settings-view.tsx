...
type Operation = 'bulkResetXp' | 'undoChildPeriod' | 'resetEggsPeriod' | 'resetAll';
...
export function SettingsView({ state,child,lang,busy,onAction,onPause,onUnpause,onSecurity,onExport,onImport,onResetAll }: {
  ...
  const eggsToReset=state.completions.filter(c=>c.child===child&&c.day>=from&&c.day<=to&&c.taskId.startsWith('egg-')&&!c.discoveryReset);
  const countEggs=eggsToReset.length,countEggXp=eggsToReset.reduce((sum,c)=>sum+c.xp,0);
  ...
  return <div className="settings-view">
    ...
    <section className="panel settings-danger">
      ...
      <div className="button-row"><Button variant="outline" disabled={busy||countXp===0} onClick={()=>setPending('bulkResetXp')}><RotateCcw size={16}/>{t(`Restablecer Xp · ${scopeTitle} (${countXp})`,`Restablir Xp · ${scopeTitle} (${countXp})`,`Reset Xp · ${scopeTitle} (${countXp})`)}</Button><Button variant="outline" disabled={busy||countEggs===0} onClick={()=>setPending('resetEggsPeriod')}><RotateCcw size={16}/>{t(`Restablecer huevos · ${scopeTitle} (${countEggs})`,`Restablir ous · ${scopeTitle} (${countEggs})`,`Reset discoveries · ${scopeTitle} (${countEggs})`)}</Button><Button variant="outline" disabled={busy||countChild+countChanges===0} onClick={()=>setPending('undoChildPeriod')}>{t(`Deshacer actividad · ${scopeTitle}`,`Desfer activitat · ${scopeTitle}`,`Undo activity · ${scopeTitle}`)}</Button></div>
