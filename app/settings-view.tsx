...
export function SettingsView({ state,child,lang,busy,onAction,onPause,onUnpause,onSecurity,onExport,onImport,onResetAll }: {
  ...
  return <div className="settings-view">
    ...
    <section className="panel settings-danger">
      ...
      <p className="muted">{t(`Estas acciones afectan a ${names[child]}. El historial mostrará cada ajuste y los bonus se recalcularán.`,`Aquestes accions afecten ${names[child]}. L’historial mostrarà els ajustos i els bonus es recalcularan.`,`These actions affect ${names[child]}. History will show the adjustments and bonuses will be recalculated.`)}</p>
      ...
      <p className="selected-period"><strong>{names[child]}</strong> · {scopeName} · {rangeLabel}</p>
      ...
      <small>{t(`Deshacer revierte las misiones que registró ${names[child]} y los cambios que hizo en propuestas o canjes durante este periodo (${countChild} misiones y ${countChanges} cambios detectados). Se conservan las correcciones posteriores hechas por padres.`,`Desfer reverteix les missions que ${names[child]} va registrar i els canvis que va fer en propostes o bescanvis durant aquest període (${countChild} missions i ${countChanges} canvis detectats). Es conserven les correccions posteriors dels pares.`,`Undo reverses missions ${names[child]} recorded and changes they made to proposals or redemptions in this period (${countChild} missions and ${countChanges} changes found). Later parent corrections are preserved.`)}</small>
    </section>
